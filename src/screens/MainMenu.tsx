...
import { HelpCircle, Play, RotateCcw, Settings, Terminal, Trophy } from "lucide-react";
    ...
    <>
      ...
      <main className="relative z-10 flex flex-col items-center justify-center w-full max-w-md px-margin-mobile">
        ...
        <header className="mb-12 text-center flex flex-col items-center">
          <Terminal className="text-[64px] text-primary mb-4" aria-hidden={true} focusable="false" />
        ...
        <div className="w-full bg-surface-container border border-outline-variant p-6 mb-8 flex flex-col items-center justify-center">
          ...
          <div className="flex items-center gap-4">
            <Trophy className="text-tertiary text-3xl" aria-hidden={true} focusable="false" />
            <span className="font-stat-value text-[48px] leading-[48px] text-on-surface font-bold">{highScoreDisplay}</span>
          </div>
        </div>
        ...
        <nav className="w-full flex flex-col gap-4">
          ...
          <button
            className="w-full h-target-min flex items-center justify-center gap-3 bg-primary text-on-primary border border-primary hover:bg-primary-container transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 focus:ring-offset-background"
            type="button"
            data-action-id="initialize-sequence-1"
            onClick={actions?.["initialize-sequence-1"]}
          >
            <Play className="fill-current" aria-hidden={true} focusable="false" />
            <span className="font-status-label text-status-label uppercase font-bold">INITIALIZE_SEQUENCE</span>
          </button>
          {/* Resume (Secondary Action) */}
          <button
            className="w-full h-target-min flex items-center justify-center gap-3 bg-surface text-on-surface border border-outline-variant hover:bg-surface-container-high hover:border-primary transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 focus:ring-offset-background"
            type="button"
            data-action-id="resume-previous-state-2"
            onClick={actions?.["resume-previous-state-2"]}
          >
            <RotateCcw className="fill-current" aria-hidden={true} focusable="false" />
          ...
          <button
            className="w-full h-target-min flex items-center justify-center gap-3 bg-surface text-on-surface border border-outline-variant hover:bg-surface-container-high hover:border-primary transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 focus:ring-offset-background"
            type="button"
            data-action-id="configure-parameters-3"
            onClick={actions?.["configure-parameters-3"]}
          >
            <Settings className="fill-current" aria-hidden={true} focusable="false" />
          ...
          <button
            className="w-full h-target-min flex items-center justify-center gap-3 bg-surface text-on-surface border border-outline-variant hover:bg-surface-container-high hover:border-primary transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 focus:ring-offset-background"
            type="button"
            data-action-id="access-documentation-4"
            onClick={actions?.["access-documentation-4"]}
          >
            <HelpCircle className="fill-current" aria-hidden={true} focusable="false" />
