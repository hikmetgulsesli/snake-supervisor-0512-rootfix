...
export function GameOptions({ actions }: GameOptionsProps) {
  ...
  const handleToggle = (key: "hapticFeedback" | "audioTelemetry" | "crtScanlines") => {
    updateSettings({ [key]: !settings[key] });
  };

  const handleResetDefaults = () => {
    actions?.["reset-defaults-4"]?.();
  };

  const handleApplyConfiguration = () => {
    actions?.["apply-configuration-5"]?.();
  };
    ...
    <>
      ...
      <nav className="hidden md:flex fixed left-0 top-0 h-full flex-col pt-16 bg-surface-container-low border-r border-outline-variant w-64 rounded-none z-40">
        ...
        <div className="flex-1 flex flex-col gap-2 w-full">
          ...
          <a
            className="flex items-center gap-4 bg-primary text-on-primary font-bold px-4 py-3 rounded-none border-l-4 border-primary"
            aria-current="page"
          >
        ...
        <div className="w-full max-w-3xl flex flex-col gap-8">
          ...
          <form className="flex flex-col gap-8">
            ...
            <section className="flex flex-col gap-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <label className="cursor-pointer relative">
                  <input className="peer sr-only" name="difficulty" type="radio" value="easy" checked={settings.difficulty === "easy"} onChange={() => handleDifficultyChange("easy")} />
                ...
                <label className="cursor-pointer relative">
                  <input className="peer sr-only" name="difficulty" type="radio" value="medium" checked={settings.difficulty === "medium"} onChange={() => handleDifficultyChange("medium")} />
                  <div className="p-4 border border-outline-variant bg-surface hover:border-primary transition-colors h-full flex flex-col gap-2 peer-checked:border-primary peer-checked:ring-2 peer-checked:ring-primary peer-checked:ring-offset-2 peer-checked:ring-offset-background">
                ...
                <label className="cursor-pointer relative">
                  <input className="peer sr-only" name="difficulty" type="radio" value="hard" checked={settings.difficulty === "hard"} onChange={() => handleDifficultyChange("hard")} />
            ...
            <div className="flex items-center justify-end gap-4 pt-8 border-t border-outline-variant mt-4">
              <button className="h-target-min px-6 border border-outline-variant bg-surface text-on-surface font-status-label text-status-label hover:border-on-surface transition-colors" type="button" data-action-id="reset-defaults-4" onClick={handleResetDefaults}>
                RESET DEFAULTS
              </button>
              <button className="h-target-min px-8 bg-primary text-on-primary font-status-label text-status-label hover:bg-surface-tint transition-colors focus:ring-2 focus:ring-primary focus:ring-offset-2 focus:ring-offset-background" type="button" data-action-id="apply-configuration-5" onClick={handleApplyConfiguration}>
                APPLY CONFIGURATION
              </button>
            </div>
          </form>
